/*
 * ADS111x driver copied from Electronic-Nose project
 * Path gốc: Source_Code/Electronic-Nose/component/ADS111x/ADS111x.c
 */
#include <esp_log.h>
        ...
        uint16_t mask)
{
    ...
    I2C_DEV_CHECK(dev, write_reg(dev, REG_CONFIG,
                                 (uint16_t)((old & ~(mask << offs)) | (val << offs))));
    I2C_DEV_GIVE_MUTEX(dev);
...
{
    return write_conf_bits(dev, (uint16_t)gain, PGA_OFFSET, PGA_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)mux, MUX_OFFSET, MUX_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)mode, MODE_OFFSET, MODE_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)rate, DR_OFFSET, DR_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)mode, COMP_MODE_OFFSET, COMP_MODE_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)polarity, COMP_POL_OFFSET, COMP_POL_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)latch, COMP_LAT_OFFSET, COMP_LAT_MASK);
}
...
{
    return write_conf_bits(dev, (uint16_t)queue, COMP_QUE_OFFSET, COMP_QUE_MASK);
}
...
esp_err_t ads111x_set_comp_low_thresh(i2c_dev_t *dev, int16_t th)
{
    ...
    I2C_DEV_TAKE_MUTEX(dev);
    I2C_DEV_CHECK(dev, write_reg(dev, REG_THRESH_L, (uint16_t)th));
    I2C_DEV_GIVE_MUTEX(dev);
...
esp_err_t ads111x_set_comp_high_thresh(i2c_dev_t *dev, int16_t th)
{
    ...
    I2C_DEV_TAKE_MUTEX(dev);
    I2C_DEV_CHECK(dev, write_reg(dev, REG_THRESH_H, (uint16_t)th));
    I2C_DEV_GIVE_MUTEX(dev);
